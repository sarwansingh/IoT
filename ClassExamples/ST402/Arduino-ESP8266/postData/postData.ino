...
SoftwareSerial WiFi_Serial(9,8); //3-Rx, 4-Tx
String str="";  boolean wf = false; 
void setup()
{ Serial.begin(115200);  // Begin serial monitor to receive 115200 bits per second (BAUD RATE)
   
  WiFi_Serial.begin(115200); // Begin serial monitor to receive 9600 bits per second (BAUD RATE)
...
{ WiFi_Serial.println("AT+CWJAP?"); delay(2000);//check if WIFI connected to any WiFi network
  ...
  { 
    ...
    { 
      ...
      str="AT+CWJAP=\"Electronics\",\"12345678\"";
      ...
      {    String RES_input = "";
        ...
        if (WiFi_Serial.find("WIFI CONNECTED")){         
            Serial.println("CONNECTED TO WIFI NETWORK..!"); delay(1000);
        }else {
...
String Device_No="Gas101" ;
String DST_IP="arshbir.com",username="st40217";
